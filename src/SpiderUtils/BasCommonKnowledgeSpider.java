...
//                通过url
        ...
        SpiderUtil.storeToDatabase(SpiderUtil.depositJavabean(title,ptime,content,"null"));
        return knowledgeSpiderConfigMiNi;
    }
    /**
    ...
    public static JXDocument listPageJsoup(JXDocument doc,KnowledgeSpiderConfigMiNi knowledgeSpiderConfigMiNi) throws XpathSyntaxErrorException, IOException {
        String nexturl=null;
        String next=null;
        JXDocument nextDocument=null;
        next=getTagOne(doc,knowledgeSpiderConfigMiNi.nextPage.getText()).toString();
        if(StringUtils.isNotEmpty(knowledgeSpiderConfigMiNi.nextPage.attributeValue("join"))){
            if (next.toString().substring(0, 4).equals("http")) {
                nexturl = next.toString().replace("..", "");
            } else {
                nexturl = knowledgeSpiderConfigMiNi.nextPage.attributeValue("join") + next.toString().replace("..", "");
            }
        }else {
            nexturl = next.toString().replace("..", "");
        }
